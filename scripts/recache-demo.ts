 ...
 *   npm run demo:recache -- --preloaded          # reuse the snapshot's map, climate and terrain layers
 *
...
function problemsWith(run: AnalysisRun): string[] {
  ...
  if (!run.simulation?.recommendedId) problems.push("the household simulation produced no project");
  else if (run.simulation.buildingSource === "sample") problems.push("the household simulation fell back to the capped building sample");
  return problems;
}

async function recache(slug: string, outDir: string, force: boolean, preloaded: boolean): Promise<boolean> {
  ...
  const t0 = Date.now();
  // Precompute settings: more sites, more futures than a live request can afford.
  const options: RunOptions = { simulation: PRECOMPUTE_OPTIONS };
  if (preloaded) {
    const prev = JSON.parse(await fs.readFile(path.join(DEMO_DIR, `${slug}.json`), "utf8")) as AnalysisRun;
    options.preloaded = { osm: prev.osm, climate: prev.climate, terrain: prev.terrain };
  }
  const run = await runAnalysis(town, emit, options);
  run.events = events;
  ...
  // The analyze route replays data/runs first, so refresh it too or the old run keeps playing.
  if (outDir === DEMO_DIR) await saveRun(run);
  ...
  if (sim?.recommendedId) {
    const best = sim.projects[0];
    console.log(`    simulation ${sim.buildingSource} · ${sim.householdClusters.toLocaleString()} clusters · ${best.label} held ${best.futures?.passed}/${best.futures?.run}`);
  }
  if (rec) {
...
async function main() {
  ...
  const preloaded = args.includes("--preloaded");
  ...
  for (const slug of slugs.length ? slugs : DEFAULT_SLUGS) results.push(await recache(slug, outDir, force, preloaded));
